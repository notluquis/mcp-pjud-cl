...
#
# Cuenta como respondido: una revisión sobre el commit de ahora, una reacción posterior al
...
filtro_pedidos='.[].data.repository.pullRequests.nodes[] | .number as $n | (.headRefOid[0:10]) as $sha | ((.comments.nodes | map(select(.body == "@codex review" or .body == "/gemini review")) | last) // empty) as $p | ([.reactions.nodes[] | select(.user.login | test("codex|gemini")) | .createdAt] | map(select(. > $p.createdAt)) | length) as $ok_reaccion | ([.reviews.nodes[] | select(.commit.oid | startswith($sha))] | length) as $ok_review | ([.comments.nodes[] | select(.author.login | test("codex|gemini")) | select(.createdAt > $p.createdAt) | .body] | map(select(contains($sha))) | length) as $ok_sha | ([.comments.nodes[] | select(.author.login | test("codex|gemini")) | select(.createdAt > $p.createdAt) | .body] | map(select(test("usage limits|reached your Codex"))) | length) as $sin_cuota | if ($ok_reaccion + $ok_review + $ok_sha) == 0 then "\($n)\t\($p.createdAt)\t\(if $sin_cuota > 0 then "cuota" else "espera" end)" else empty end'
